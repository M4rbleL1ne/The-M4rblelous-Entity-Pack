...
public class MiniLeviathanGraphics : BigEelGraphics
{
    public MiniLeviathanGraphics(MiniLeviathan ow) : base(ow)
    {
        ...
        var tl = tail = new TailSegment[12];
        ...
        {
            finsData[j, 0] = 5f + 10f * Mathf.Sin(Mathf.Pow((float)j / (fns.Length - 1), .5f) * Mathf.PI);
    ...
    public override void InitiateSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
    {
        ...
        if (eel is MiniLeviathan be)
        {
            var sprites = sLeaser.sprites;
            var alt = be.abstractCreature.superSizeMe;
            sprites[MeshSprite].shader = Custom.rainWorld.Shaders[alt ? "MiniLeviEelBody" : "AMiniLeviEelBody"];
            var flg = fins.Length;
            for (var k = 0; k < 2; k++)
            {
                for (var l = 0; l < flg; l++)
